...
export function useTodos(defaultTodos: Todo[]) {
  ...
  const addDraft = (todo: Omit<Todo, "id" | "created_at">) => {
    const draft = {
      ...todo,
      id: DRAFT_ID,
      created_at: new Date().toISOString(),
  ...
  const appendDraft = (todo: Omit<Todo, "id" | "created_at">) => {
    const newTodo = {
      ...todo,
      id: Date.now(),
      created_at: new Date().toISOString(),
  ...
  const updateTodoStatus = (id: number, status: TodoStatus) => {
    ...
    const previousStatus: TodoStatus = todo?.status;
    if (
        ...
        return (
          <div className="flex items-center gap-6 w-[300px] pl-1 font-mono">
            <p className={"text-sm  font-medium"}>
              Updated <span className="font-medium">"{todo.title}"</span> status
              to {status}
            </p>
            <Button
              className="ml-auto"
